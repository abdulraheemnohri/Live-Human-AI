...
public:
    ...
    bool initialize() { return m_engine.initialize(); }
    void shutdown() { std::lock_guard<std::mutex> lock(m_mutex); m_engine.shutdown(); m_activeLoop = 0; }
    ...
    bool start() { std::lock_guard<std::mutex> lock(m_mutex); return m_activeLoop > 0 && m_engine.startLoop(m_activeLoop); }
    bool pause() { std::lock_guard<std::mutex> lock(m_mutex); return m_activeLoop > 0 && m_engine.pauseLoop(m_activeLoop); }
    bool resume() { std::lock_guard<std::mutex> lock(m_mutex); return m_activeLoop > 0 && m_engine.resumeLoop(m_activeLoop); }
    bool cancel() { std::lock_guard<std::mutex> lock(m_mutex); return m_activeLoop > 0 && m_engine.cancelLoop(m_activeLoop); }
    ...
    Decision process(const Input& input) {
        ...
        d.confidenceDecision = JalebiConfidencePolicy::decide(
            input.confidence, input.evidenceAvailable, false, degraded);
    ...
    JalebiLoopEngine::Iteration execute(const std::string& input) {
        std::lock_guard<std::mutex> lock(m_mutex);
        return m_activeLoop > 0 ? m_engine.executeIteration(m_activeLoop, input) : JalebiLoopEngine::Iteration{};
    }
    ...
    bool evaluate(float confidence, bool completed, const std::string& evidence,
        ...
        std::lock_guard<std::mutex> lock(m_mutex);
        return m_activeLoop > 0 && m_engine.recordEvaluation(m_activeLoop, confidence, completed, evidence, nextAction, memoryUpdates);
    }
