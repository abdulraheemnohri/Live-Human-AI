...
jstring stringToJstring(JNIEnv* env, const std::string& str) { return env->NewStringUTF(str.c_str()); }
...
JNIEXPORT void JNICALL Java_com_livehumanai_livehumanai_nativebridge_NativeBridge_nativeShutdown(JNIEnv*, jobject, jlong nativeHandle) {
    ...
    if (core) { core->shutdown(); delete core; }
...
JNIEXPORT jstring JNICALL Java_com_livehumanai_livehumanai_nativebridge_NativeBridge_nativeExecuteJalebiIteration(JNIEnv* env,jobject,jint id,jstring input){if(id!=g_jalebiRuntime.activeLoop())return stringToJstring(env,"[]");g_jalebiRuntime.execute(jstringToString(env,input));return stringToJstring(env,g_jalebiRuntime.engine().getLoopHistoryJson(id));}
JNIEXPORT jboolean JNICALL Java_com_livehumanai_livehumanai_nativebridge_NativeBridge_nativeEvaluateJalebiLoop(JNIEnv* env,jobject,jint id,jfloat confidence,jboolean complete,jstring evaluation,jstring nextAction,jstring memoryUpdates){if(id!=g_jalebiRuntime.activeLoop())return JNI_FALSE;return g_jalebiRuntime.evaluate(confidence,complete==JNI_TRUE,jstringToString(env,evaluation),jstringToString(env,nextAction),jstringToString(env,memoryUpdates))?JNI_TRUE:JNI_FALSE;}
JNIEXPORT jstring JNICALL Java_com_livehumanai_livehumanai_nativebridge_NativeBridge_nativeGetJalebiLoopState(JNIEnv* env,jobject,jint id){if(id!=g_jalebiRuntime.activeLoop())return stringToJstring(env,"IDLE");auto&e=g_jalebiRuntime.engine();return stringToJstring(env,e.getStateName(e.getLoopState(id)));}
