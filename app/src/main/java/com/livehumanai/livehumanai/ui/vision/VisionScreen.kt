...
fun VisionScreen(viewModel: AIViewModel = hiltViewModel()) {
    ...
    DisposableEffect(permissionGranted,lifecycleOwner){
        if(permissionGranted){
            val pv=preview
            if(pv!=null){
                cameraManager.initialize(pv,lifecycleOwner)
                cameraManager.setupFrameAnalysis{frame->viewModel.submitJalebiCameraFrame(frame)}
    ...
    Column(Modifier.fillMaxSize().padding(12.dp),verticalArrangement=Arrangement.spacedBy(8.dp)){
        ...
        if(!permissionGranted){
            ...
            AndroidView(factory={ctx->PreviewView(ctx).also{preview=it}},modifier=Modifier.fillMaxWidth().weight(1f).aspectRatio(16f/9f))
